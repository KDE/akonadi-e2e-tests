...
@pytest.mark.asyncio
async def test_list_calendars(dav_client: DavClient, groupware_resource: DAVResource) -> None:
    server_calendars = await dav_client.list_calendars()
    # Skip root collection
    akonadi_calendars = [c for c in await groupware_resource.list_collections() if c.parent_id != 0]
